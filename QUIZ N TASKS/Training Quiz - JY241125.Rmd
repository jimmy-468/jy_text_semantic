...
# Your code here
data_list_named <-  data_list %>% 
  set_names(str_remove(basename(files_list), "\\.csv"))
...
mdn_df <- wide_df %>%
  ungroup() %>% 
  group_by(Date) %>% 
  mutate(Mdn_value = median(c_across(A:C), rm.na = T))
mdn_df
...
#processed_df <- rename_with(processed_df, ~paste0(.x, '_raw'), all_of(c("A", "B", "C")))

processed_df <- processed_df %>% rename_at(vars(A:C), ~paste0(., '_raw'))
